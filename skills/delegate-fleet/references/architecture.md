...
- **Visibility of untracked-and-ignored writes.** Repository facts come from `git status`, so a
  write to a gitignored path or outside the repository is not observable.
